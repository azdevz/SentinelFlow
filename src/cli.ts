...
async function collectLiveCIEvidence(
  ...
  const repoFull = process.env.GITHUB_REPOSITORY || 'azdevz/SentinelFlow';
  ...
  try {
    // Try git diff against base branch if in git repo
    diff = execSync(`git diff origin/${baseBranch}...HEAD || git diff HEAD~1 || git diff`, {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'ignore'],
    });

    const fileListRaw = execSync(`git diff --name-only origin/${baseBranch}...HEAD || git diff --name-only HEAD~1 || git status -s`, {
      encoding: 'utf-8',
      stdio: ['pipe', 'pipe', 'ignore'],
    });
    changedFiles = fileListRaw.split('\n').map((f) => f.trim()).filter(Boolean);
  } catch (err: any) {
    console.warn(`[SentinelFlow CI] Warning collecting git diff: ${err.message}`);
  }

  // Ingest test-results.json if generated by CI
  let testResults: TestRunResult | undefined;
  if (fs.existsSync('test-results.json')) {
    try {
      const raw = fs.readFileSync('test-results.json', 'utf-8');
      testResults = collector.parseTestResults(raw);
    } catch (e: any) {
      console.warn(`[SentinelFlow CI] Could not parse test-results.json: ${e.message}`);
    }
  }

  // Ingest audit-results.json if generated by CI
  let auditJson: any = undefined;
  if (fs.existsSync('audit-results.json')) {
    try {
      const rawAudit = fs.readFileSync('audit-results.json', 'utf-8');
      auditJson = JSON.parse(rawAudit);
    } catch (e: any) {
      console.warn(`[SentinelFlow CI] Could not parse audit-results.json: ${e.message}`);
    }
  }

  const pr: PRMetadata = {
    owner: owner || 'azdevz',
    repo: repo || 'SentinelFlow',
    pullNumber,
    commitSha,
    title: process.env.GITHUB_PR_TITLE || `Pull Request #${pullNumber}`,
    baseBranch,
    headBranch: process.env.GITHUB_HEAD_REF || 'feature',
  };

  const securityResults = scanner.evaluate(diff, auditJson);
